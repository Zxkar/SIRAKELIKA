...
<div class="flex min-h-screen">
    ...
    <main class="main-content" style="padding-top: 0;">
        ...
        <section id="kontak" class="section-anchor mb-10">
            <h2 class="text-base font-bold text-gray-800 mb-2">📞 Kontak & Bantuan</h2>
            <p class="text-xs text-gray-400 mb-6">Butuh bantuan atau ingin berkonsultasi? Hubungi kami melalui saluran berikut.</p>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                <!-- Kontak 1: Email -->
                <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-5 flex flex-col gap-3">
                ...
                <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-5 flex flex-col gap-3">
                    ...
                    <div>
                        <h4 class="text-sm font-bold text-gray-800 mb-1">Kantor Kemahasiswaan</h4>
                        <p class="text-xs text-gray-500 mb-3">Kunjungi langsung di Gedung Rektorat Lantai 2, Institut Teknologi B.J. Habibie, Parepare.</p>
                        <span class="text-xs font-bold text-orange-500">Senin–Jumat, 08.00–16.00 WITA</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
                <h3 class="text-sm font-bold text-gray-800 mb-1">Kirim Pesan / Konsultasi</h3>
                <p class="text-xs text-gray-400 mb-5">Isi formulir di bawah ini untuk menghubungi tim SIRAKELIKA. Pesan Anda bersifat rahasia.</p>

                <?php if (!empty($konsultasi_success)): ?>
                <div class="bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-semibold rounded-xl px-4 py-3 mb-4 flex items-center gap-2">
                    <i class="fa fa-check-circle"></i> <?= htmlspecialchars($konsultasi_success) ?>
                </div>
                <?php endif; ?>
                <?php if (!empty($konsultasi_error)): ?>
                <div class="bg-red-50 border border-red-200 text-red-600 text-xs font-semibold rounded-xl px-4 py-3 mb-4 flex items-center gap-2">
                    <i class="fa fa-circle-exclamation"></i> <?= htmlspecialchars($konsultasi_error) ?>
                </div>
                <?php endif; ?>
                ...
                <form method="POST" action="#kontak">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="text-xs font-semibold text-gray-600 block mb-1.5">Nama <span class="text-gray-400 font-normal">(opsional)</span></label>
                            <input type="text" name="nama" placeholder="Nama Anda atau tulis 'Anonim'" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        </div>
                        <div>
                    ...
                    <div class="mb-4">
                        <label class="text-xs font-semibold text-gray-600 block mb-1.5">Pesan</label>
                        <textarea name="pesan" rows="4" required placeholder="Tuliskan pertanyaan atau ceritakan situasi yang Anda hadapi..." class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none"></textarea>
                    </div>
                    <div class="flex justify-between items-center">
                        <p class="text-[11px] text-gray-400"><i class="fa fa-lock mr-1 text-gray-300"></i>Pesan Anda dijaga kerahasiaannya</p>
                        <button type="submit" name="kirim_konsultasi" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold transition">
